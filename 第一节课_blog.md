...
### 1.1 问题：为什么多核下代码会“乱跑”？
在单核时代，并发主要源于中断和抢占。但在多核（SMP）环境下，即便你禁用了本地中断，代码依然可能产生非预期的结果。你必须时刻假设，当前正在操作的任何一个内存变量，都可能在下一个时钟周期被中断流、其他物理核心或 DMA 设备篡改。
...
**事实是：MESI 协议在硬件层面是同步且阻塞的。** 当核心 A 试图写入一个共享缓存行时，它必须发出失效请求并等待所有核心的 Ack 应答，这会严重卡住流水线。为了榨取性能，硬件引入了两个微架构队列：
1. **存储缓冲区 (Store Buffer)**：核心 A 执行写操作时，不再等待其他核心的应答，而是将新值直接推入私有的 Store Buffer，随后立刻执行下一条指令。
2. **失效队列 (Invalidate Queue)**：核心 B 收到失效请求后，不再立刻清除缓存，而是先丢进队列并立刻回复应答，承诺“等会儿再删”。
