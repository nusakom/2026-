...
#### 强内存模型（如 x86 / x86_64）
硬件遵循 **TSO (Total Store Order)** 内存模型，默认提供较强的顺序保证，唯一需要显式屏障的场景是防止 `Store → Load` 重排：
- `sfence`：保证写操作顺序（Store Fence）
- `lfence`：保证读操作顺序（Load Fence）
- `mfence`：全屏障（Memory Fence），阻止所有类型的重排序
> 在 x86 上，`memory_order_seq_cst` 通常只需插入 `mfence` 即可实现；而 `acquire/release` 往往无需额外指令（因硬件 TSO 天然支持）。
...
通过 `fence` 指令实现，其语法为 `fence pred, succ`：

```asm 
fence rw, rw # 全屏障 (Full Barrier)
fence r, rw  # Acquire 语义 (Load 之后插入屏障)
fence rw, w  # Release 语义 (Store 之前插入屏障)
``` 
...
**破解方案：从静态定义到手工孵化**
1. **阶段一：静态定义的“鸡”（0 号进程）**
   在 Linux 中，第一个进程 `init_task` 是在内核编译时**静态定义**的。它不通过 fork 产生，而是内核代码段的一部分。它负责早期的硬件初始化（调度、内存、中断）。
2. **阶段二：手工创建的“蛋”（1 号进程胚胎）**
   0 号进程通过 `kernel_thread` 手工设置 CPU 寄存器和内核栈，创建出一个内核线程。此时它还不是用户态进程，而是 1 号进程的“胚胎”。
3. **阶段三：从内核态到用户态的“蜕变”**
   该线程执行 `kernel_init`，最后通过 `execve` 加载用户程序（如 `/sbin/init`）。`execve` 会销毁原有的内核态映射，建立用户态内存空间，并跳转到程序入口。至此，PID=1 的用户态进程正式诞生。
4. **阶段四：2 号进程与收尾**
